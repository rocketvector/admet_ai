    ...
    def __init__(
        ...
        if isinstance(mol, str):
            if self.is_reaction:
                ...
                mol = make_mol(
                    mol, self.is_explicit_h, self.is_adding_hs, self.is_keeping_atom_map
                )
        ...
        if not self.is_reaction:
            ...
            for a1 in range(self.n_atoms):
                for a2 in range(a1 + 1, self.n_atoms):
                    if (
                        a1 >= n_atoms_reac and a2 >= n_atoms_reac
...
class BatchMolGraph:
    ...
    * :code:`b2br`: (Optional): A mapping from f_bonds to real bonds in molecule recorded in targets.
    ...
    def get_components(self, atom_messages: bool = False) -> Tuple[
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
...
def atom_features_zeros(atom: Chem.rdchem.Atom) -> List[Union[bool, int, float]]:
    ...
    if atom is None:
        ...
        features = onek_encoding_unk(
            ...
            PARAMS.ATOM_FDIM - PARAMS.MAX_ATOMIC_NUM - 1
